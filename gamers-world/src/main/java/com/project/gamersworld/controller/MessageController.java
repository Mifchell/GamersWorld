...
public class MessageController {
    ...
    public String editMessage(HttpServletRequest request, @RequestParam(value = "message") String message,
        ...
        return "redirect:/messages/user/" + mRepo.findByMessageID(messageID).getSender().getUserID();
    }

    @PostMapping("/message/delete")
    public String deleteMessage(HttpServletRequest request, @PathVariable("messageID") int messageID) {
        int messSenderID = mRepo.findByMessageID(messageID).getSender().getUserID();
        mHandler.deleteMessage(messageID);
        return "redirect:/messages/user/" + messSenderID;
    }
}
